...
(function () {
    ...
    var loadChat = function (cb) {
        ...
        $.get("https://rawgit.com/darkunderdog/basicBot/master/lang/lang.json", function (json) {
    ...
    var botCreator = "Matthew (darkunderdog)";
    ...
    var basicBot = {
        ...
        chatLink: "https://rawgit.com/darkunderdog/basicBot/master/lang/en.json",
        chat: null,
        loadChat: loadChat,
        ...
        settings: {
            ...
            newwinner: "/me :fireworks: New Winner: @%%CWNAME%% - Song: %%CWMEDIA%%-%%CWSONGNAME%% - Woot Ratio: %%RATIO%% - :+1:%%CWWOOTS%% :family:%%AUDIENCE%% :punch:%%CWCURATES%% :-1:%%CWMEHS%%",
			currentwinner: "/me :sparkler: Current Winner: @%%CWNAME%% - Song: %%CWMEDIA%%-%%CWSONGNAME%% - Woot Ratio: %%RATIO%% - :+1:%%CWWOOTS%% :family:%%AUDIENCE%% :punch:%%CWCURATES%% :-1:%%CWMEHS%%",
            ...
            commandLiteral: "!",
            blacklists: {
                NSFW: "https://rawgit.com/darkunderdog/basicBot-customization/master/blacklists/ExampleNSFWlist.json",
                OP: "https://rawgit.com/darkunderdog/basicBot-customization/master/blacklists/ExampleOPlist.json"
            }
        },
        room: {
            users: [],
        ...
        eventDjadvance: function (obj) {
			...
			if (basicBot.room.roomstats.CWMinAudience <= API.getUsers().length) {
				if (basicBot.room.roomstats.CWRatio === lastplay.score.positive/API.getUsers().length && basicBot.room.roomstats.CWCurates < lastplay.score.grabs) {
					...
					basicBot.room.roomstats.CWMedia = lastplay.media.author;
					...
					API.sendChat(subChat(basicBot.settings.newwinner, {cwname: basicBot.room.roomstats.CWName, cwmedia: basicBot.room.roomstats.CWMedia,cwsongname: basicBot.room.roomstats.CWSongName, ratio: basicBot.room.roomstats.CWRatio.toFixed(2), cwwoots: basicBot.room.roomstats.CWWoots, audience: basicBot.room.roomstats.CWAudience ,cwcurates: basicBot.room.roomstats.CWCurates, cwmehs: basicBot.room.roomstats.CWMehs}));
				}
			...
			if (basicBot.settings.songstats) {
				if (typeof basicBot.chat.songstatistics === "undefined") {
					API.sendChat("/me " + lastplay.media.author + " - " + lastplay.media.title + ": " + lastplay.score.positive + "W/" + lastplay.score.grabs + "G/" + lastplay.score.negative + "M/" + API.getUsers().length + "A.")
				}
				else {
					API.sendChat(subChat(basicBot.chat.songstatistics, {artist: lastplay.media.author, title: lastplay.media.title, woots: lastplay.score.positive, grabs: lastplay.score.grabs, mehs: lastplay.score.negative, audience: API.getUsers().length}))
				}
			}
        ...
        chatUtilities: {
            ...
            commandCheck: function (chat) {
                ...
                if (chat.message === '!roulette') {
                    if (userPerm < 2) {
                    	var u = basicBot.userUtilities.lookupUser(chat.uid);
                        var diff = (60*60) - (((Date.now() - basicBot.room.roulette.rouletteInterval) / 1000) | 0);
        		var minutes = (diff / 60) | 0;
        		var seconds = (diff % 60) | 0;
                        if (basicBot.room.roulette.rouletteInterval !== null && (Date.now() - basicBot.room.roulette.rouletteInterval) < 1 * 60 * 60 * 1000) {
                            API.moderateDeleteChat(chat.cid);
                            API.sendChat("[!roulette] @" + u.username + " you must wait " + minutes + ":" + seconds + " minutes.");
                            return void (0);
                        }
            ...
            CurrentWinnerCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!basicBot.commands.executable(this.rank, chat)) return void (0);
						...
						var ratio = basicBot.room.roomstats.CWRatio;
						var audience = basicBot.room.roomstats.CWAudience;
						var media = basicBot.room.roomstats.CWMedia;
						if (currentwinner === "" || currentwinner === null) {
							API.sendChat("/me No Current Winner");
                    	}
                    		else {
                    			API.sendChat(subChat(basicBot.settings.currentwinner, {cwname:currentwinner, cwmedia: media,cwsongname: song, ratio: ratio.toFixed(2), cwwoots: woots, audience: audience, cwcurates: grabs, cwmehs: mehs}));
                    		}
            ...
            CWReset: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!basicBot.commands.executable(this.rank, chat)) return void (0);
						...
						basicBot.room.roomstats.CWMedia = "";
